...
# White-specific: Dual Spatial Filtering
HEAD_X_MAX = 0.25      # Left 25%
HEAD_Y_MAX = 0.30      # Top 30%
BOTTOM_Y_MIN = 0.75    # Bottom 25%

# Morphology (with dilation for white)
KERNEL_SIZE = 7
...
def visualize_pipeline(input_path, output_dir):
    ...
    # Morphology (White-specific: includes dilation)
    kernel = np.ones((KERNEL_SIZE, KERNEL_SIZE), np.uint8)
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel, iterations=2)
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel, iterations=1)
    mask = cv2.dilate(mask, kernel, iterations=1)  # White-specific dilation
    ...
    for i, contour in enumerate(contours):
        ...
        # Filter 1: Head Region (upper-left)
        in_head_region = (x < width * HEAD_X_MAX and 
                          y < height * HEAD_Y_MAX)
        ...
        # Filter 2: Bottom Reflection (lower area)
        center_y = y + h/2
        in_bottom_region = (center_y > height * BOTTOM_Y_MIN)
        
        if in_bottom_region:
            rejected_objects.append({
                'bbox': (x, y, w, h),
                'reason': 'Bottom Reflection'
            })
            continue
    ...
    head_count = sum(1 for obj in rejected_objects if obj['reason'] == 'Head Region')
    bottom_count = sum(1 for obj in rejected_objects if obj['reason'] == 'Bottom Reflection')
    density_count = sum(1 for obj in rejected_objects if obj['reason'] == 'High hole density')
    
    if head_count > 0 or bottom_count > 0:
        print(f"  - Head Region: {head_count}")
        print(f"  - Bottom Reflection: {bottom_count}")
        print(f"  - High hole density: {density_count}")
    ...
    if len(valid_objects) > 0:
        ...
        # Draw all valid objects (gray)
        for obj in valid_objects:
            if obj == selected_obj:
                continue
            x, y, w, h = obj['bbox']
            cv2.rectangle(img_bbox, (x, y), (x+w, y+h), (128, 128, 128), 2)  # Gray
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python visualize_pipeline_white.py <input_image> [output_dir]")
        print("\nExample:")
        print("  python visualize_pipeline_white.py input.jpg visualization_white/")
        print("\nWhite-specific features:")
        print("  - Dual spatial filtering:")
        print("    1) Head region exclusion (upper-left 25%×30%)")
        print("    2) Bottom reflection exclusion (Y>75%)")
        print("  - High V_min threshold (150) for bright achromatic material")
        print("  - Additional dilation morphology for edge preservation")
        sys.exit(1)
    
    input_path = sys.argv[1]
    output_dir = sys.argv[2] if len(sys.argv) > 2 else "./visualization_white"
