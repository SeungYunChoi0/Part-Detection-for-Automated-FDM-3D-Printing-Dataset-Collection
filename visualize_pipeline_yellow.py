#!/usr/bin/env python3
"""
Pipeline Visualization for Yellow Filament
...
def visualize_pipeline(input_path, output_dir):
    ...
    for i, contour in enumerate(contours):
        ...
        # Spatial filtering (Yellow has no specific exclusions)
        # All objects that pass geometric filtering are considered valid
    ...
    for obj in rejected_objects:
        x, y, w, h = obj['bbox']
        # Draw dashed rectangle (simulate with multiple lines)
        dash_length = 10
        color = (0, 0, 255)  # Red
        thickness = 2
    ...
    if len(valid_objects) > 0:
        ...
        # Draw all valid objects (yellow)
        for obj in valid_objects:
            if obj == selected_obj:
                continue
            x, y, w, h = obj['bbox']
            cv2.rectangle(img_bbox, (x, y), (x+w, y+h), (0, 255, 255), 2)  # Yellow
...
def create_combined_visualization(output_dir, stem):
    ...
    FIXED: Ensures all images have equal width for consistent alignment.
    """
    output_dir = Path(output_dir)
    ...
    # Calculate target width (use average of a,b,c for consistency)
    widths = [img_a_resized.shape[1], img_b_resized.shape[1], img_c_resized.shape[1]]
    target_width = int(np.mean(widths))
    
    def resize_and_center(img, target_w, target_h):
        """Resize or pad image to exact target dimensions (centered)"""
        h, w = img.shape[:2]
        
        # First, resize to fit within target dimensions while maintaining aspect ratio
        ...
        # Now add padding if needed to reach exact target dimensions
        if w < target_w or h < target_h:
    ...
    # Apply uniform width
    img_a_uniform = resize_and_center(img_a_resized, target_width, target_height)
    img_b_uniform = resize_and_center(img_b_resized, target_width, target_height)
    img_c_uniform = resize_and_center(img_c_resized, target_width, target_height)
    img_d_uniform = resize_and_center(img_d_resized, target_width, target_height)
    ...
    img_a_labeled = add_label(img_a_uniform, "(a) Input")
    img_b_labeled = add_label(img_b_uniform, "(b) HSV Mask")
    img_c_labeled = add_label(img_c_uniform, "(c) Spatial Filtering")
    img_d_labeled = add_label(img_d_uniform, "(d) Output")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python visualize_pipeline_yellow.py <input_image> [output_dir]")
        print("\nExample:")
        print("  python visualize_pipeline_yellow.py input.jpg visualization_yellow/")
        sys.exit(1)
    
    input_path = sys.argv[1]
    output_dir = sys.argv[2] if len(sys.argv) > 2 else "./visualization_yellow"
