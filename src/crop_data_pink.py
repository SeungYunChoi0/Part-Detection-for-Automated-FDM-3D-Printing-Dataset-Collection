...
# 설정
# ============================================================
INPUT_DIR = Path('/Volumes/Yun_ssd/Modulino_CV/dataset_norm_pink')
OUTPUT_DIR = Path('/Volumes/Yun_ssd/Modulino_CV/data_cropped_pink')
FAILED_DIR = OUTPUT_DIR / 'failed'
LOG_FILE = OUTPUT_DIR / 'processing_log.txt'
SUMMARY_FILE = OUTPUT_DIR / 'summary.json'

# HSV 색상 범위 (핑크/마젠타) - 빨간색 제외
LOWER_PINK = np.array([140, 50, 60])  # S 30→50, V 50→60
UPPER_PINK = np.array([165, 255, 255])  # H 170→165 (빨간색 제외)
...
def detect_pink_part(img):
    ...
    for i, contour in enumerate(contours):
        ...
        if MIN_AREA < area < MAX_AREA:
            ...
            # 오른쪽 하단 스티커 영역 제외
            # 스티커는 보통 x > width*0.65, y > height*0.65 영역에 위치
