...
INPUT_DIR = Path('/Volumes/Yun_ssd/Modulino_CV/dataset_norm_blue')
OUTPUT_DIR = Path('/Volumes/Yun_ssd/Modulino_CV/data_cropped_blue')
...
def process_images():
    ...
    for idx, img_path in enumerate(image_files, 1):
        ...
        try:
            ...
            print(f"[{idx}/{total_images}] ERROR: {img_path.name} - {str(e)}")
...
if __name__ == "__main__":
    try:
        ...
        if stats['total'] > 0 and (stats['success'] / stats['total']) < 0.9:
            print("\n 경고: 성공률이 90% 미만입니다. 파라미터 조정이 필요할 수 있습니다.")
