#!/usr/bin/env python3
"""
RGB-based White Batch Cropping Script (No HSV conversion)
RGB 기반 흰색 출력물 일괄 크로핑 스크립트
...
# 설정
# ============================================================
INPUT_DIR = Path('/Volumes/Yun_ssd/Modulino_CV/dataset_norm_white')
OUTPUT_DIR = Path('/Volumes/Yun_ssd/Modulino_CV/data_norm_white_cropped_RGB')
...
# RGB 색상 범위 (흰색)
# ============================================================
# White in RGB: R high, G high, B high (all similar values)
# OpenCV uses BGR order!
LOWER_WHITE_RGB = np.array([150, 150, 150])  # [B_high, G_high, R_high]
UPPER_WHITE_RGB = np.array([255, 255, 255])  # [B_max, G_max, R_max]
...
def detect_white_part(img):
    ...
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    mask = cv2.inRange(img, LOWER_WHITE_RGB, UPPER_WHITE_RGB)
    ...
    # Dilation: 경계 확장 (White 특수 - 높은 임계값으로 인한 경계 손실 방지)
...
def process_images():
    ...
    print(f"RGB 임계값: LOWER={LOWER_WHITE_RGB}, UPPER={UPPER_WHITE_RGB}")
    print(f"{'='*60}\n")
    ...
    stats = {
        ...
        'special_processing': 'Dilation + Dual spatial filtering (head + bottom)'
    }
    ...
    with open(LOG_FILE, 'w') as f:
        ...
        f.write(f"RGB Range: LOWER={LOWER_WHITE_RGB}, UPPER={UPPER_WHITE_RGB}\n")
        f.write(f"Special: Dilation + Head/Bottom filtering\n")
        f.write("="*60 + "\n\n")
    ...
    # HSV vs RGB 비교 메시지
    print(f"\n📊 RGB 기반 파이프라인 사용 (White 특수 처리)")
    print(f"   - HSV 변환 없이 직접 RGB 마스킹 수행")
    print(f"   - Dilation 적용 (경계 손실 방지)")
    print(f"   - 헤드 영역 제외 (x<25%, y<30%)")
    print(f"   - 하단 영역 제외 (y>75%)")
